...
if (!SEPOLIA_RPC) {
  console.error('❌ ERROR: SEPOLIA_RPC_URL is not set in environment variables!');
  process.exit(1);
}

if (!ARTIINA_NFT_ADDRESS) {
  console.error('❌ ERROR: ARTIINA_NFT_ADDRESS is not set in environment variables!');
  process.exit(1);
}
...
// Initialize contract
const artiinaNFT = new ethers.Contract(ARTIINA_NFT_ADDRESS, ARTIINA_NFT_ABI, provider);
...
app.get('/:beadId', async (req, res) => {
  try {
    ...
    const normalizedBeadId = beadId.toUpperCase();

    // Check if bead exists
    const beadExists = await artiinaNFT.exists(normalizedBeadId);
    if (!beadExists) {
    ...
    const beadData = await artiinaNFT.beads(normalizedBeadId);
    const metadata = await artiinaNFT.getBeadMetadata(normalizedBeadId);
    const tokenId = await artiinaNFT.beadToToken(normalizedBeadId);
    const transferCount = await artiinaNFT.getTransferCount(normalizedBeadId);
...
app.listen(PORT, () => {
  ...
  console.log(`📡 Connected to RPC (hidden for security)`);
  console.log(`📝 Contract: ${ARTIINA_NFT_ADDRESS}`);
